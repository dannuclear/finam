...
public interface RESTMapper {
    ...
    // Series toDto(TradeGroupReferenceAsset domain, List<Bar> bars, Map<String,
    // Object> extraParams);
    ...
    // @Mapping(target = "panelNum", source = "domain.panelIndex")
    // @Mapping(target = "extraParams", ignore = true)
    // Series toDto(AnalysisAsset domain, List<Bar> bars, List<TradeHistory.Trade>
    // trades);

    @Mapping(target = "id", source = "domain.asset.symbol")
    @Mapping(target = "name", source = "domain.asset.name")
    @Mapping(target = "panelNum", source = "domain.panelIndex")
    ChartBarSeries map(AnalysisAsset domain, List<Bar> values);

    @Mapping(target = "id", source = "domain.asset.symbol")
    @Mapping(target = "name", source = "domain.asset.name")
    @Mapping(target = "panelNum", source = "domain.panelIndex")
    ChartBarSeriesWithTrades map(AnalysisAsset domain, List<Bar> values, List<TradeHistory.Trade> trades);

    // @Mapping(target = "name", source = "domain.asset.name")
    // @Mapping(target = "panelNum", source = "domain.panelIndex")
    // Series toDto(AnalysisAsset domain, List<Bar> bars, Map<String, Object>
    // extraParams);
}
