...
public interface ProtobufStandardMappings {
    ...
    default com.google.type.Date mapLocalDate(LocalDate t) {
        return t != null
                ? com.google.type.Date.newBuilder().setYear(t.getYear()).setMonth(t.getMonthValue())
                        .setDay(t.getDayOfMonth()).build()
    ...
    default com.google.type.TimeOfDay mapLocalTime(LocalTime t) {
        return t != null
                ? com.google.type.TimeOfDay.newBuilder().setHours(t.getHour()).setMinutes(t.getMinute())
                        .setSeconds(t.getSecond()).setNanos(t.getNano()).build()
    ...
    default Timestamp map(LocalDateTime i) {
        ...
        int offset = systemDefault.getOffset(GregorianCalendar.AD, i.getYear(), i.getMonthValue() - 1,
                i.getDayOfMonth(), i.getDayOfWeek().getValue(),
                i.getNano() / 1000);

        return Timestamp.newBuilder().setSeconds(i.toEpochSecond(ZoneOffset.ofTotalSeconds(offset / 1000)))
                .setNanos(i.getNano()).build();
    }

    default Timestamp map(OffsetDateTime in) {
        return in != null ? Timestamp.newBuilder().setSeconds(in.toEpochSecond()).setNanos(0).build()
    ...
    default BigDecimal map(Decimal f) {
        if (f == null) {
            return null;
        }
