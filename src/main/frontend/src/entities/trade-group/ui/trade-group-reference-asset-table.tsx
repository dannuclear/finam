...
export type TradeGroupReferenceAssetTableProps = {
    rows: TradeGroupReferenceAsset[],
    onAdd?: () => void,
    onEdit?: (id: string) => void,
    onDelete?: (id: string) => void
}

export const TradeGroupReferenceAssetTable = ({ rows, onAdd, onEdit, onDelete }: TradeGroupReferenceAssetTableProps) => {
    return (<>
            ...
            <Table size="small" aria-label="a dense table">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell width={1}>Смещение</TableCell>
                ...
                <TableBody>
                    {rows.filter(row => row.changeStatus !== "DELETED").map((row) => (
                        <TableRow key={row._tempId}>
                            ...
                            <TableCell component="th" scope="row">{row.enabled}</TableCell>
                            ...
                            <TableCell component="th" scope="row">{row.priceOffset}</TableCell>
                            <TableCell component="th" scope="row">{row.panelNum}</TableCell>
                            <TableCell component="th" scope="row">
